...
class BMGCN(nn.Module):
    def __init__(self, mlp_module, gcn_module, loss_weight, feat_balance, score_balance, device):
    ...
    def forward(self, feature, adj, idx, label, labels_oneHot, train_idx):
        ...
        H = self.get_block_matrix(adj, labels_oneHot, B.clone(), train_idx)

        score_S = torch.mm(torch.mm(B, S), B.t()) * adj
        score_H = torch.mm(torch.mm(B, H), B.t()) * adj
        score = self.score_balance * score_S + (1 - self.score_balance) * score_H
    ...
    def get_block_matrix(self, adj, y, soft_y=None, mask=None):
        soft_y[mask] = y[mask]
